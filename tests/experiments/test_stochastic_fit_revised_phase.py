...
def _moment_test_config(**kw) -> RP.FitConfig:
    """A small front end for the moment-stage focused tests."""
    base = dict(
        n_fft=256,
        hop=128,
        sr=SR,
        ...
        temperature=1.0,
        bias_std_hz=0.5,
        bias_mean_std_hz=RP.BIAS_MEAN_PRIOR_STD_HZ,
        log_d_mean=0.0,
        log_d_std=2.0,
        atom_dtype="float64",
        moments=RP.MomentConfig(window=512, hop=128, lags=(2, 4, 8, 16)),
        gate=RP.MomentGate(
            min_line_snr_db=5.0,
            min_isolation_bins=4.0,
            order_range=(2, 3),
            min_frames=4,
            max_wrap_spread_rad=1.2,
        ),
        provenance={},
    )
    base.update(kw)
...
def _synthetic_harmonic_clip(
    ...
    *,
    ...
    phase_raw = 2.0 * np.pi * np.cumsum(rps, axis=1) / sr
...
def test_shared_shaft_is_identified_by_cross_order_covariance():
    """Independent per-order OU processes can be tuned to reproduce the SAME
    diagonal variances as a shared shaft.  Only the off-diagonal covariance
    exposes the shared motion; this test pins that the moment stage fits a
    positive shared ``sigma`` from cross-order structure."""
    rng = np.random.default_rng(7)
    T = 2 * SR
    f0 = 200.0
    rps = np.full((1, T), f0, dtype=np.float64)
    lam_true, sigma_true = 15.0, 3.0
    dt = 1.0 / SR
    innov = rng.standard_normal((T, 2))
    theta, _ = RP.simulate_state(innov, lam=lam_true, sigma=sigma_true, dt=dt)
    clip = _synthetic_harmonic_clip(rps, {2: theta, 3: theta}, d=0.05, seed=7)
    cfg = _moment_test_config()
    dynamics = RP.estimate_shaft_dynamics([("clip", clip)], gate=cfg.gate, config=cfg)

    assert dynamics.identified, dynamics.diagnostics.get("unidentifiable_reason")
    assert dynamics.sigma == pytest.approx(sigma_true, rel=0.4)
    assert dynamics.lam == pytest.approx(lam_true, rel=0.5)
    cross = dynamics.diagnostics["crossorder_cells"]
    assert cross["n_cells"] > 0
    assert all(c["cov"] > 0.0 for c in cross["cells"])


def test_independent_per_order_ou_does_not_support_shared_k_load():
    """With independent per-order OU paths that have the SAME marginal
    variances as the shared case, the diagonal structure is identical but the
    cross-order covariance is noise.  The moment stage must NOT report a large
    shared ``sigma`` from diagonals alone."""
    ...
    lam_true, sigma_true = 15.0, 3.0
    dt = 1.0 / SR
    theta2, _ = RP.simulate_state(
        rng.standard_normal((T, 2)), lam=lam_true, sigma=sigma_true, dt=dt
    )
    theta3, _ = RP.simulate_state(
        rng.standard_normal((T, 2)), lam=lam_true, sigma=sigma_true, dt=dt
    )
    clip = _synthetic_harmonic_clip(rps, {2: theta2, 3: theta3}, d=0.05, seed=8)
    cfg = _moment_test_config()
    dynamics = RP.estimate_shaft_dynamics([("clip", clip)], gate=cfg.gate, config=cfg)

    cross = dynamics.diagnostics["crossorder_cells"]
    assert cross["n_cells"] > 0
    max_cov = max(abs(c["cov"]) for c in cross["cells"])
    assert max_cov < 0.3
    # The off-diagonal shared-shaft fit must not recover the per-order sigma.
    assert dynamics.sigma < 1.0


def test_changing_carrier_with_zero_residual_does_not_manufacture_shared_variance():
    """A ramping carrier with zero shaft phase noise demodulates to zero
    residual increments.  The old frame-mean/nominal-advance subtraction could
    create artificial residual variance; the within-window integration must not.
    """
    T = 2 * SR
    t = np.arange(T) / SR
    f0 = 150.0 + 100.0 * t / t[-1]  # ramp 150-250 Hz, separation > 4 bins
    rps = f0[None, :].astype(np.float64)
    clip = _synthetic_harmonic_clip(
        rps, {2: np.zeros(T), 3: np.zeros(T)}, d=0.0, seed=9
    )
    cfg = _moment_test_config()
    dynamics = RP.estimate_shaft_dynamics([("clip", clip)], gate=cfg.gate, config=cfg)
    ...
    max_cov = max(abs(c["cov"]) for c in cross["cells"])
    assert max_cov < 0.05
    assert dynamics.sigma < 0.5


def test_cross_order_covariance_intersects_valid_pair_supports():
    """Cross-order covariance is computed only over frame pairs that are valid
    for BOTH orders.  A gap at a different location in each order's mask must
    leave only the overlapping pairs."""
    rng = np.random.default_rng(3)
    lo_a = np.arange(0, 20, dtype=np.int64)
    hi_a = lo_a + 1
    pid_a = RP._pair_id(lo_a, hi_a)
    res_a = rng.normal(0.0, 0.1, size=20)

    lo_b = np.arange(10, 30, dtype=np.int64)
    hi_b = lo_b + 1
    pid_b = RP._pair_id(lo_b, hi_b)
    res_b = rng.normal(0.0, 0.1, size=20)

    cov, n_common = RP._aligned_covariance(res_a, pid_a, res_b, pid_b)
    assert n_common == 10
    expected = float(np.mean(res_a[10:20] * res_b[:10]))
    assert cov == pytest.approx(expected, rel=1e-12)

...
round 3b. Here the round-2 fit and the legacy anchor fit of each rig are
added.[^listen-legacy]

[^listen-legacy]: The legacy row is `noise_lab.LegacyFit` of the anchor
    the legacy banks were drawn around, the one of @fig-i2: DREGON
    `dregon_cruise_refined` (clip `free-flight_nosource_room2_cruise_00`),
    Michael's `michael_cruise` (clip `fly125_cruise_00`). Dynamics `donor`,
    the class default: the donor policy's per-clip draw, as one bank entry
    at zero perturbation, line mode `fm`, speed exponents bounded. It is the
    acoustic build a training stream renders, not the point-preset run.
    Mic 0 of its 8. `scripts/noise_v3_latent_runaway_figs.py --round r3b
    --listen-add legacy` renders it into `figdata_r3b.json` beside the
    other clips.
...
![**What you see:** DREGON, `free-flight_nosource_room2`, 50–60 s, mic 0. Rows: the real recording, the legacy DREGON anchor fit (`dregon_cruise_refined`, donor dynamics), the v2 fit, the round-2 and the round-3b v3 fits, all on the real rotor track (bottom), same RMS, one colour scale.](noise-model-v3-latent-runaway/fig_j_listen_dregon_r3b.png){#fig-j3}

```{python}
listen_players(F3["r3b"], "dregon", R3_LABELS)
```

Legacy DREGON lines stand tallest: $k = 1$ 14.9 dB, $k = 2$ 7.9 dB, real
4.4 and 2.9 dB. Legacy is also too quiet everywhere, 2.3–6.5 dB low in
every band, so its LTAS distance, 4.3 dB, is the worst of the four fits.
<!-- source: figdata_r3b.json listen.rigs.dregon.clips.legacy.line_over_floor_db[0:2] 14.9249, 7.9041; clips.real 4.4401, 2.8916; clips.legacy.ltas_mean_abs_db 4.2702, ltas_dev_db [-2.64, -2.32, -3.22, -5.05, -6.51, -5.87]; v2 0.518, v3 2.160, v3_r3b 3.610 -->
...
![**What you see:** Michael's, `FLY124`, 40–50 s, mic 0. Rows as in @fig-j3; the legacy row is the Michael's anchor fit (`michael_cruise`, donor dynamics).](noise-model-v3-latent-runaway/fig_k_listen_michaels_r3b.png){#fig-k3}

```{python}
listen_players(F3["r3b"], "michaels", R3_LABELS)
```

Legacy Michael's has $k = 1$ at 7.4 dB where real has none; its $k = 2$
is 11.5 dB, real 18.7 dB. Legacy is 1–3 dB too loud in every band; its
LTAS distance, 2.1 dB, ties round 3b (2.0 dB).
<!-- source: figdata_r3b.json listen.rigs.michaels.clips.legacy.line_over_floor_db[0:2] 7.3634, 11.5179; clips.real null, 18.6767; clips.legacy.ltas_mean_abs_db 2.0604, ltas_dev_db [2.30, 3.00, 1.93, 0.95, 1.14, 3.04]; v3_r3b 2.0472, v2 1.266, v3 2.934 -->
...
hard bank (@fig-i3). When this section was asked for, no v2 or v3 arm
came close to it on the real validation. Then the round-3b easy arm
finished. It is the first synthetic-only arm below the legacy pair.
...
- Dmitrii reports: legacy hard also wins on the static and stochastic
  validation parts.
- Round 3b beats legacy on the easy bank: 4.68 against 6.09 and 5.41.
  The real reference is 3.11.
- Round 3b loses on the hard bank: 7.79. Hard is the open question.
...
[^arms]: `docs/experiments/noise-model-v3.md` § Training arms (round-2
    fits) and (round-3 fits) § Results: `val/real_r3`, PIT MAE, raw at
    the selected round. Jobs `nv3-easy-scv2-9bc4c1`,
    `nv3-hard-scv2-149094`, `nv3r3-easy-scv2-fd6e96`,
    `nv3r3-hard-scv2-5ea979`.

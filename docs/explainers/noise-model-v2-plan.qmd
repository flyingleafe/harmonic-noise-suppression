...
: C3 values from `docs/experiments/revised-phase-handoff-2026-09-13.qmd:139-142`, at the fixed `λ_ref = 6 s⁻¹`. Bench acoustics from `shaft.json:acoustics.dregon_bench`; bound and wobble from the two tables above. The corpus-wide label-residual bound is `D_θ ≤ 0.41 rad²/s` and `Var θ_res = 2.5e-5` to `6.8e-3 rad²`. On Michael's the study measures `D_θ = 0.0256 rad²/s` in band, which is `σ = 0.392 rad/s` at `λ_ref = 6` against C3's fitted 4.21 — a factor 10.7 (`shaft/findings.md:70-73`).
...
Read `σ_ν`, `λ` and `D_θ` from this instrument; do not read `D_k` from it. The
prior on `log D_g` therefore still cannot come from an acoustic `V_k`
decomposition, and `p_g = 0` stays fixed in R1 and R2 by default.
...
| `λ` | fixed at `λ_ref = 6 s⁻¹`, a reference assumption | `λ_L` **free per label chain**, prior centred on the label-free bench measurement 5.48 s⁻¹ |
| `σ` | free, prior `log σ ~ N(0, 2²)` | `σ_L` free, prior centred on the bench 1.77 rad/s; the above-band `σ_ν` is fixed from telemetry |
...
The OU block has **two free parameters, not three**: the fit samples
`(log σ_L, log λ_L)` and `D_θ,L = σ_L²/λ_L` follows. The centre comes from the
label-free bench, de-biased by the planted control; the width has to reach the
label-dominated regime as well, because what part (i) describes on a labelled
rig is the shaft error **plus** whatever the label chain adds.

| parameter | prior | central 95 % | derived from |
|---|---|---|---|
| `log σ_L` | `N(ln 1.77, 0.95²)` | 0.28-11.4 rad/s | centre: the bench median 1.77 rad/s (de-biased 1.4, band 1.1-1.8). Width: reaches the bench full range 0.23-3.20 and the label-residual `ν_res` RMS of 2.7-11.3 rad/s |
| `log λ_L` | `N(ln 5.5, 1.50²)` | 0.29-103 s⁻¹ | centre: the bench median 5.48 s⁻¹ (de-biased 1.8-5.5). Width: covers the bench full range 0.31-16.2 and the lower half of the label-residual corner band 38-485 s⁻¹ |
| `D_θ,L = σ_L²/λ_L` | implied | 0.57 rad²/s at the centre | the bench median is 0.599 (IQR 0.38-0.66, de-biased 0.6-1.2); the implied centre agrees to 5 % |
...
: Proposed priors. Sources: `results/noise_v2/shaft/shaft.json` (`acoustics.dregon_bench`, `planted_control`, `label_band`) via `results/noise_v2/shaft/findings.md`, and `results/noise_v2/criteria/findings.md:81-87`.

Three consequences are worth stating, because they are what the priors buy.

1. **`λ_ref = 6 s⁻¹` sits 0.06 prior standard deviations from the centre.**
   The C2/C3 assumption is not overturned; it is absorbed as the
   prior mean, and R1 can fix `λ` there for the parity test without fighting
   the prior.
2. **A fit that lands above `λ_L ≈ 40 s⁻¹` is a diagnosis, not a number.**
   That is the label-residual regime, which means the label chain and not the
   shaft dominates the residual on that arm — exactly the DREGON room-2
   situation.
3. **`D = 844.69 rad²/s` is unreachable under this prior**, at
   `ln(844.69/0.3)/0.8 = 9.9` prior standard deviations. The C3 DREGON fit
   cannot recur by accident.

The prior on `log D_g` still cannot come from the acoustic `V_k`
decomposition. The bench now measures the shaft term, but `D_k` itself remains
unidentified on both rigs, so the line-shape bound is what is left, and that is
a weaker instrument: an upper limit from one rig at two orders.
...
- **`(λ_L, σ_L)` free per label chain.** Proposal: free in R2 with the
  ...
  Two facts push this way. The one label-free instrument centres the prior but
  does not pin it: the DREGON bench measures `λ = 5.48 s⁻¹` and
  `σ_ν = 1.77 rad/s`, and the planted control says the estimator recovers `λ`
  at 3.07×, so the true value may sit anywhere in 1.8-5.5 s⁻¹. And the
  parameters describe the label chain — command, sample-and-hold measured, or
  refined — not controller physics, so one rig with two labels has two values.
  Decide whether the pair is shared across a rig's label arms or fitted per
  arm.
...
- **The `D_k` order structure.** Proposal `D_gk = D_g·k^{p_g}` with
  `p_g = 0` fixed in R1 and R2. The corrected bench acoustics now identify the
  shaft term on 12 of 12 recordings, but `D_k` itself is still unidentified:
  the fitted `2D_kτ` at the shortest lag is 1.32 times the measured `V` of the
  same order, and the planted control returns a planted `D_k ∝ k` as flat.
  So no bench evidence forces `p_g` free.

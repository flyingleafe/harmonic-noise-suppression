...
| `γ_rk` (Hz) | `gamma_hz` | per rotor × order | bench; flight (Michael's) | LN(`0.01·k` Hz, 1.0); `γ ≥ 0` | **tight and physically anchored** (user decision 2026-09-18): the bench decoherence law `V_ε ≈ 0.09 k^1.1 τ^0.43` rad² is an effective diffusion `≈ 0.1k` rad²/s, i.e. HWHM `≈ 0.01k` Hz; one log-sd is ×2.7, so a line may be 7× wider or narrower than that law at 2σ but cannot absorb the shaft core (Gaussian, `≈ kσ_ν/2π ≈ 0.05k` Hz at `σ_ν = 0.3` rad/s). On 128 ms flight frames (bins 7.8 Hz) this prior dominates below `k ≈ 50`, which is intended. Initialised from each line's measured −3 dB width, clipped to ±2σ of the prior |

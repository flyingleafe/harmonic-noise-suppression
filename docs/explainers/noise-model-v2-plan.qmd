...
10. The reviewer must settle nine open choices: the window, the proxy, the
    two thresholds, which orders enter the DREGON likelihood given the label
    precision `k_max(T)`, whether `(λ_L, σ_L)` are free per label chain, the
    above-band wobble, the `D_k` order structure, and the DREGON label arm.
    Section [Decisions for the reviewer](#decisions-for-the-reviewer) lists
    them.
...
Bench points with four rotors carry **four constant carriers as fit
parameters**, initialised from the survey's per-rotor split. The priors above
are unchanged by that: the carriers are per-point nuisance parameters, not
...
Michael's, and the survey's 18 bench cells — is drawn from a population
...
- **Which orders enter the DREGON likelihood, given the label precision.**
  On DREGON room-2 cruise the label holds only `k = 1` inside `±0.5 Hz` at
  `T = 4.096 s`, and only on 31 % of cells; at `k ≥ 2` it is 0-6 %. Two causes
  are mixed. The label's own error `δ` displaces harmonic `k` by `k·δ`, and
  the 38.5 Hz per-rotor isolation band already holds the neighbouring rotors'
  lines of the same order at `k ≥ 2`. C4 activates all `K = 130` DREGON
  orders. Decide whether v2 keeps them, restricts the DREGON likelihood to the
  orders below `k_max(T)`, or scores the high orders only through the LTAS
  proxy.
...
- **The fixed above-band wobble.** Proposal: take `(σ_ν, λ)` per rig from the
  ...
  `2σ_ν²λ/(3πω_c³)`, checked against the two measurements — `S_max/2` from the
  shaft study and the integrated residual of the residual study. The wobble is
  small: rms `0.003-0.055 rad` at `k = 1`, `0.09-1.65 rad` at `k = 30`. The
  residual study also shows what to generate, a filtered noise with an `f⁻²`
  PSD above the band edge and a 100 ms correlation length, and not white
  noise. Decide whether R1 carries it at all, or whether it enters with the
  high orders in R2.

...
telemetry gives `λ = 0.67–14.4 s⁻¹` (identified rigs) and the DREGON bench
acoustics `σ_ν ≈ 1.8 rad/s`, `λ ≈ 5.5 s⁻¹`, `D_θ = σ_ν²/λ ≈ 0.6 rad²/s`. The
...
diffusion. This term replaces the C2–C4 per-harmonic Wiener `ε_rk` and its
`k`-independent `D`. A `k`-independent floor is kept only as a nuisance bound
from the bench (`D < 1.45 rad²/s`) and is not a default parameter.

**Total phase**

$$\phi_{mrk}(t)=2\pi k\int_0^t f_r(s)\,ds+k\,\theta_r(t)+k\,\eta_r(t)+\alpha_{mrk}.$$
...
$$V_\theta(\tau)=2\sigma_\nu^2\left[\frac{|\tau|}{\lambda}-\frac{1-e^{-\lambda|\tau|}}{\lambda^2}\right],
\qquad
R_{rk}(\tau)=\exp\!\left[-\tfrac{k^2}{2}V_\theta(\tau)-k^2\sigma_p^2\bigl(1-\rho_p(\tau)\bigr)\right].$$

Both widths scale with `k`: Gaussian shoulders `kσ_ν/2π`, Lorentzian core
`k²D_θ/2π`, path sidebands of width `λ_p/2π` and depth `k²σ_p²`. That scaling
is what makes the high orders the measurement of the dynamics: a width of one
7.8 Hz bin at `k = 10` is 0.78 Hz at `k = 1`.
...
| per-harmonic phase | Wiener `ε_rk`, `D_g` with no `k` scaling | replaced by a shared short-`τ` OU on the phase, `(σ_p, λ_p)`, sidebands `∝ k` |
...
|---|---|---|---|
| `log σ_ν` | `N(ln 1.77, 0.6²)` | 0.55-5.7 rad/s | bench median 1.77 rad/s (de-biased 1.4, band 1.1-1.8); width covers the bench full range 0.23-3.20 |
...
`(log σ_ν, log λ, log σ_p, log λ_p, floor shape, profile slope)` for every rig

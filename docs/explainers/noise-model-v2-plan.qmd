...
10. Decided at review: DREGON is fitted on the bench (total periodogram) and
    ...
    front-end STFT; the shaft is one OU process; the flat `D` is replaced by a
    shared short-`τ` path OU. Seven choices remain open: the path term in R1,
    Michael's standby/ramp, the proxy, the two thresholds, the bench
    stationarity rule, and the four-motor validation tolerance.
...
1. **The independent term is real and large.** Power law over `k ∈ [4, 40]`,
   `τ ∈ [10, 500] ms`: `V_ε = 0.09 · k^1.08 · τ^0.43` rad² (`τ` in s; CI
   `a` 0.05-0.38, `p` 0.64-1.44, `q` 0.36-0.74; 2000 bootstrap draws over
   recordings). It exceeds the shared shaft term at every order above 4.
2. **It is independent across orders**: mean off-diagonal correlation 0.03
   over 630 order pairs at 50 ms, with no odd/even block. The parity shows in
   the variance: each odd order carries 3.7× the independent noise of its even
   neighbours at 50 ms (median 0.77 vs 0.23 rad²), and the two families scale
   differently in lag — even `q = 0.88` (still diffusive at 500 ms), odd
   `q = 0.46` (saturating).
3. **It is mostly source-side**: cross-microphone correlation 0.21, against
   0.00 for a noise-only control and 0.30 for a control with the term fully
   shared across microphones.
4. **The shaft term is smaller than first fitted.** The shared part alone
   gives `V_θ(50 ms) = 7.9e-4`, `V_θ(500 ms) = 0.017 rad²`, i.e.
   `σ_ν ≈ 0.45 rad/s` at `λ = 5.5` (0.31-0.65 across recordings). The earlier
   bench fit of `σ_ν = 1.77` had absorbed the per-order term into the shaft.
...
What it means for the model: the per-harmonic term the first draft of this
page dropped comes back as an **independent per-order process on the
phase**, driven `∝ k^{1/2}`, written as an OU with a correlation time of
0.3-1 s in the simplest form, one pair `(σ_ε, λ_ε)` per parity (section
[Model v2](#model-v2)). The OU's ceiling is an inference from sub-linear
growth, not an observation: no plateau is seen inside the 500 ms lag range.
R1 extends the lag range to 1-5 s on the long bench recordings and replaces
the OU by a rate-modulated random walk if the variance keeps growing.
...
`D` is replaced by a **second shared OU on the shaft phase with a short
correlation time**. The fit protocol changed with it: DREGON is fitted on the
...
**Path phase — a second shared OU, short correlation time, fitted.** Mic
motion, frame vibration and wake turbulence change the source-to-mic path
length `δr(t)`; the phase they add to order `k` is `2πf·δr/c ∝ k`, shared by
every harmonic of the rotor like the shaft term, with a correlation time of
milliseconds to ~100 ms instead of 0.1–1 s:

$$d\eta_r=-\lambda_p\,\eta_r\,dt+\sqrt{2\lambda_p\sigma_p^2}\;dW_r^{(p)},
\qquad \lambda_p\gg\lambda .$$

`η_r` is a phase-per-order (rad at `k = 1`); it is bounded (OU on the phase
itself, not on its rate), so it adds `k`-proportional sidebands, not a `k²`
diffusion. It is shared by all orders of a rotor.
...
$$\phi_{mrk}(t)=2\pi k\int_0^t f_r(s)\,ds+k\,\theta_r(t)+k\,\eta_r(t)+\varepsilon_{rk}(t)+\alpha_{mrk}.$$
...
$$V_\theta(\tau)=2\sigma_\nu^2\left[\frac{|\tau|}{\lambda}-\frac{1-e^{-\lambda|\tau|}}{\lambda^2}\right],
\qquad
R_{rk}(\tau)=\exp\!\left[-\tfrac{k^2}{2}V_\theta(\tau)-k^2\sigma_p^2\bigl(1-\rho_p(\tau)\bigr)-\sigma_\varepsilon^2k^{p}\bigl(1-e^{-\lambda_\varepsilon|\tau|}\bigr)\right].$$

Every width scales with `k`: Gaussian shoulders `kσ_ν/2π`, Lorentzian core
`k²D_θ/2π`, path sidebands of width `λ_p/2π` and depth `k²σ_p²`, and the
per-order term of width `λ_ε/2π` and depth `σ_ε²k^p`. That scaling is what
makes the high orders the measurement of the dynamics: a width of one 7.8 Hz
bin at `k = 10` is 0.78 Hz at `k = 1`. On the bench the per-order term is
the largest of the three above `k ≈ 4` (ratio to the shaft term 1-5 at
`k ≥ 10`, 50 ms).
...
| per-harmonic phase | Wiener `ε_rk`, `D_g` with no `k` scaling | independent per-order OU `ε_rk` with strength `∝ k^{1/2}` and correlation time `1/λ_ε` (measured on the bench), plus a shared short-`τ` path OU `(σ_p, λ_p)` |
...
| `log σ_p` | `N(ln 0.03, 1.5²)` | 0.002-0.6 rad at `k = 1` | **unmeasured**; centre from a 1 mm path change at 340 m/s at `f = 80 Hz` (`2π·80·0.001/340 ≈ 1.5e-3` rad) times a margin; the bench fit in R1 measures it |
| `log λ_p` | `N(ln 30, 1.0²)` | 4-210 s⁻¹ | **unmeasured**; ms-100 ms correlation times of vibration and wake turbulence; must stay above `λ` |
...
`(log σ_ν, log λ, log σ_p, log λ_p, log σ_ε, log λ_ε, floor shape, profile slope)` for every rig
...
- **Decided at review (2026-09-16), recorded, no longer open:** DREGON is
  ...
  one OU process; the `k`-independent `D` is replaced by a shared short-`τ`
  path OU. The DREGON label arm is moot (no free-flight fit).
- **The per-order term's form.** The bench measures an independent per-order
  phase term `V_ε = 0.09·k^1.1·τ^0.43` rad², odd orders 2-4× the even ones,
  even orders diffusive to 500 ms and odd saturating. Proposal: one OU per
  order, driven `∝ k^{1/2}`, one `(σ_ε, λ_ε)` pair per parity. Alternative:
  a single pair with `p` free, or the C2-C4 Wiener form with `D_k ∝ k`
  (which cannot saturate). Decide.
- **The path term in R1.** Proposal: fit `(σ_p, λ_p)` on the bench from R1,
  with the wide priors above. Alternative: fix `σ_p = 0` in R1 (pure shaft
  model) and free it in R2 only if the bench residual at high orders shows
  sidebands the shaft cannot make. Decide.
- **Michael's standby and ramp.** Proposal: not fitted; rendered from the
  bench dynamics through the trajectory sampler, still scored by the frozen
  gate. Alternative: keep FLY125 standby in the flight fit for the speed law
  at low rates. Decide.
...
- **The likelihood threshold.** The oracle (Welch mean periodogram of a
  speed-matched disjoint real segment, one spectrum held fixed over the
  scored support) is a **stationary** predictor: inside a support the carrier
  moves and the oracle's lines do not. A model that predicts the dynamics from
  the label frame by frame must **beat** it wherever the label is accurate to
  better than one bin. Reference values at the flight STFT (NFFT 2048, hop
  512, `results/noise_v2/short_whittle/findings.md`): the old flight-fitted
  model already sits at the oracle on Michael's cruise (−107 nats/s) and 66 k
  nats/s above it pooled (standby, 14 dB hot); the old bench-fitted model is
  3.97 M nats/s above it on DREGON cruise. Proposal: gate = composite risk
  **below the oracle** on Michael's cruise by a margin frozen in R1 from the
  parity fit; DREGON free flight is not gated by this likelihood (label error
  ~1 Hz at `k = 70` removes the dynamic advantage at high order) but by HPPNet
  and LTAS. Decide whether the margin is per support or pooled.
- **The floor in flight, and the band of the likelihood gate.** Below
  ~200-300 Hz the DREGON cruise spectrum is wind, not rotor. A model whose
  floor was fitted on a clamped motor in a quiet room sits 20-30 dB under it
  and pays `I/M` of 100-1000 per cell (the tail concentration of
  `results/noise_v2/short_whittle/findings.md`); the oracle carries the wind
  for free. Proposal: the coloured floor (level, colour, slow variation) is a
  flight quantity fitted on the flight support with the comb frozen from the
  bench, and the likelihood gate is reported **per band** — comb band and
  floor band — so a pass on the comb band is what counts. The rendered
  training noise needs the flight floor regardless. Decide the band edge and
  whether the floor level may vary per frame.
- **The stationarity rule on the bench.** Proposal: the longest sub-segment
  where the demodulated residual at `k ≈ 70` stays inside ±1 Hz
  (`utils.demod.residual_frequency`). Alternative: a slow polynomial carrier
  inside the periodogram atom over the whole record. Decide.
- **Four-motor validation tolerance.** Proposal: the four-motor fit passes
  when `|Δ log σ_ν|, |Δ log λ|, |Δ log D_θ|` lie inside the per-rotor spread
  of the single-motor fits, with widths allowed larger and low orders flagged.
  The numeric spread is frozen from the R1 per-rotor fits. Decide whether that
  is acceptable as a criterion frozen after R1's first stage.

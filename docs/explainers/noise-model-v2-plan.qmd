...
- **The likelihood threshold.** Proposal: the same closure fraction on the
  composite risk at the flight STFT (NFFT 2048, hop 512), oracle = speed-matched
  disjoint real segment. The absolute values come from
  `scripts/noise_v2_short_whittle.py` (previous-generation model at the short
  STFT), once run. Decide whether R1 blocks on that table, and whether the

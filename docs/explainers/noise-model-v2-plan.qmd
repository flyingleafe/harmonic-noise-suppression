...
page dropped comes back as an **independent per-order OU on the phase**,
driven `∝ k^{1/2}`, with a correlation time of 0.1-1 s, one pair
`(σ_ε, λ_ε)` per parity in the simplest form (section [Model v2](#model-v2)).
...
even ones by 2-4×. A process with that lag law is an OU on the phase with a
correlation time of 0.1-1 s, driven with a strength `∝ k`:

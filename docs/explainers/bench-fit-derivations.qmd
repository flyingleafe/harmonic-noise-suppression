...
model floor that cannot represent the value it measures. See §10, items 1, 2 and 8.
:::
...
Statistics alone do not close a stage. The five bench cells were rendered from
the fitted vector through the SAME renderer the training streams use, and
decimated on the real clips' own path. Each panel is real, fitted, and a draw
from the untouched default family, plus the LTAS band profile of all three.

|cell|rev/s|fitted LTAS mean\|dev\| (dB)|pre-fit (dB)|
|---|---:|---:|---:|
|Motor1_50|49.0|2.06|7.25|
|Motor1_60|58.8|1.94|9.69|
|Motor1_70|68.5|1.93|9.62|
|Motor1_80|78.2|2.51|10.59|
|Motor1_90|88.0|2.03|8.67|

::: {layout-ncol=1}
![Motor 1 at setpoint 80](stage1-bench/bayes_Motor1_80.png)

![Motor 1 at setpoint 50](stage1-bench/bayes_Motor1_50.png)

![Motor 1 at setpoint 90](stage1-bench/bayes_Motor1_90.png)
:::

The fitted panel carries the real clip's comb, its 4.3 kHz and 6.5 kHz bright
bands, and its broadband floor level. The pre-fit panel does not: its comb is
too strong, its floor too dark, and its LTAS falls away by more than 10 dB
across the band.

The residual is in the 3 to 7 kHz bands, where the fitted LTAS sits about 4 dB
low. WAV files for every cell are in `docs/explainers/stage1-bench/` as
`bayes_Motor1_NN_{real,fitted,prefit}.wav`.
...
cells = [r["cell"] for r in idx] if (idx := locals().get("idx")) else [
    f"Motor1_{s}" for s in (50, 60, 70, 80, 90)
]
html = ["<table style='width:100%;border-collapse:collapse'>",
        "<tr><th style='text-align:left'>cell</th><th>real</th><th>fitted</th>"
        "<th>pre-fit</th></tr>"]
for c in cells:
    tds = "".join(
        f"<td style='padding:4px'><audio controls preload='metadata' "
        f"style='width:230px' src='stage1-bench/bayes_{c}_{k}.wav'></audio></td>"
        for k in ("real", "fitted", "prefit")
    )
    html.append(f"<tr><td style='padding:4px'><b>{c}</b></td>{tds}</tr>")
html.append("</table>")
_d(HTML("\n".join(html)))
```
...
# 10. Hacks, defects, and hindrances

...
::: {layout-ncol=1}
![refit, cruise window 0](stage2-cruise/cruise_refined_00.png)

![refit, cruise window 1](stage2-cruise/cruise_refined_01.png)

![refit, cruise window 2](stage2-cruise/cruise_refined_02.png)
:::
...
::: {layout-ncol=1}
![DREGON refit, free-flight window 0](stage2-cruise/dregon_refined_00.png)

![DREGON refit, hovering window 0](stage2-cruise/dregon_refined_01.png)
:::
...
if dregon_panels:
    ...
    _d(
        HTML(
            "<p><em>The DREGON refit has no panels in this checkout. Build them "
            "with <code>python scripts/_stage2_panels.py --fit "
            "results/S2/dregon_room2_cruise_refined.json --dataset DREGON-frames "
            "--recording free-flight_nosource_room2 --prefix dregon_refined "
            "--clips 2 --seconds 8</code>.</em></p>"
        )
    )
```
...
the RENDER path, and it had been there for every synthetic clip this project has
made.
...
order) grid. The render now **low-passes itself before decimation**, with a
filter whose passband (7900 Hz, the fit's own band edge), stopband (8000 Hz, the
output Nyquist) and attenuation (100 dB) are stated, and whose length follows
from that specification through `kaiserord`. `stage2.antialias`, applied in both
`stage2.render_from_export` and `stage1_bayes.render_from_export`.

Measured after the fix, same window, same seed: the decimated 7.5–8 kHz band
falls from 15.1 to 2.5 dB with the above-8 kHz contribution at −148 dB, the
6–8 kHz band goes 11.6 → 7.9 dB against the real clip's 7.8 dB, and everything
below 5 kHz is bit-for-bit what it was. The tone test now reads −4.9 dB at
7.9 kHz (identical to the unfiltered path, so nothing the fit modelled is lost)
and −163 dB at 8.1 kHz.
...
speed law needs it. Fit the carrier offset (`rps_offset`, already in the model)
on ramp clips, and report it.
...
19. **Aliasing in every render, found on 2026-09-12 and fixed.** The renderer
    synthesised comb lines up to 10.8 kHz at 44.1 kHz and the shared decimator's
    default Kaiser stop band (40-50 dB) folded them into the top octave: 14.5 dB
    of the decimated 7.5-8 kHz band came from above 8 kHz, against 5.7 dB of
    genuine content, and a 9 kHz tone arrived only 30.6 dB down. Fixed by a
    render-side low-pass with a stated specification (`stage2.antialias`,
    7900 Hz pass / 8000 Hz stop / 100 dB). Two lessons: the tell was a bright
    band that only ONE of two otherwise similar fits showed, and the reason it
    showed there was an unconstrained boundary order (+6.8 dB against −10.9 dB
    for its neighbours) — **an order outside the fit band is not measured, and
    rendering it is extrapolation with a resampler as the only thing standing
    between it and the output.** Every gate number recorded before that date was
    measured on clips carrying this artefact.

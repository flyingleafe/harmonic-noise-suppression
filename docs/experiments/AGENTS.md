...
| `telemetry-fitness.md` | — (standalone driver `scripts/telemetry_fitness.py`, no conf/experiment entry) | The goodness-of-fit HARNESS for a candidate rotor-speed trajectory (issue #17 phases 6a-6e): FOUR components at fixed degrees of freedom, held-out harmonics/channels/time, all four §B controls, residual decomposition + bootstrap. **Read § "Phase 6d" first if you are citing a number**: the first three components are shares of one in-band power and saturate; the ridge component is the one that reads a line against a local floor. § "Phase 6e" adds the TIME axis — DREGON's telemetry runs early by -42 ms [-85,-31] (a tachometer lag is excluded by sign) |
...
| `stochastic-fit.md` | — (fit driver `scripts/stochastic_fit.py --regime {cruise,standby,bench}`; population/gate/export pipeline `src/experiments/stochastic_fit/run.py`; no conf/experiment entry) | Whittle MAP fit of the stochastic rotor-noise family to real recordings with fixed RPS: how much of a real recording the family as rendered explains at its best parameters (74 % on DREGON room2, 87 % on room1/FLY124/FLY125), where it fails (the line's fall-off — a Gaussian halves the remaining misfit on three of four rigs, not room1; partly coherent low harmonics on Michael's rig; heavier-than-exponential amplitudes; per-mic level structure), and which structural relaxation buys what. Explainer: `docs/explainers/stochastic-fit.qmd` |
| `rig-sampler-transfer-pair.md` | `rig_{easy,hard}_scv2_unified`, `rig_easy_hppnet_l2_unified` (banks built by `scripts/_build_rig_bank.py`) | Easy and hard sim-to-real transfer (2026-09-14) on the STOCHASTIC family: neighbourhoods of the two rigs' cruise fits instead of the fits as points. Widening helps and the wide path cloud is not worse (hard 5.37, easy 5.72 best real MAE vs 9.97 point-preset, 2.99 real audio); the residual gap is all in the transitions, where both arms collapse the four rotors onto one speed. **Read § "Open decision: rebuild the banks" BEFORE launching any `rig_*` arm or rebuilding `data/rig_banks/`** |
| `noise-v2-transfer.md` | `nv2_{easy,hard,mixed}_{scv2,hppnet_l2}`, `nv2_{easy,hard}_ft_{scv2,hppnet_l2}` (banks built by `scripts/noise_v2_build_bank.py --preset easy|hard`; submission `scripts/noise_v2_submit_arms.sh`) | Sim-to-real transfer on the FITTED noise model (2026-09-22, configs landed, **nothing submitted**): the v2 fits sampled as easy neighbourhood / hard cruise-path banks, flown on FITTED trajectories, against the legacy stochastic pair (5.72 / 5.37) and the real references (2.99 SCv2, 2.27 HPPNet-L2), with curriculum and mixed arms on both trunks. The hard/mixed streams fly the rig hyperprior TRUNCATED by `rps_max: 150` (acceptance 0.296; it truncates hover level, not rig diversity — read § "The trajectory cap" before citing a trajectory statistic) |
| `rps-trajectory-model.md` | — (campaign code `src/experiments/rps_traj/`, drivers `scripts/rps_traj_*.py`; no conf/experiment entry) | A per-rig GENERATIVE model of rotor-speed trajectories, fitted to 7 rigs, judged by a frozen five-family rule. Read § "What the telemetry says" BEFORE citing a band or ACF number: DREGON `motor.measured` above 5 Hz is its own 45 Hz staircase. Sum-of-OU is excluded (R2, 0/7); OU + CAR(2) reaches the shape (R3, 1/7); **best = R4**, Kalman likelihood + coloured per-rotor measurement term (michaels 5/5; dregon/pitcn/blackbird 4/5). Flown in training by `rps.kind: fitted_traj` (see `noise-v2-transfer.md`) |
| `noise-model-v2.md` | — (studies `scripts/noise_v2_{bench_speed,corpus_survey,shaft_phase,likelihood_window,spectrogram_proxy}.py`; no conf/experiment entry) | PRE-IMPLEMENTATION of noise model v2 (2026-09-15): the three studies behind the successor of revised-phase C1-C4. Survey adds 73 fit points over 12 rig/condition points (`noise-v2-bench-points@8f49bb0f77d3`), validated against the DREGON throttle law to 0.555 rev/s; the label-free bench acoustics measure the shaft term on 12/12 recordings, so C3's `D = 845 rad²/s` is LABEL error, not rotor physics, and `D_k` stays UNIDENTIFIED. **Read § "Shaft-phase outcome" before quoting any `λ`** — it is BRACKETED, not pinned (a factor 77 across trend-removal variants), and the label-residual diffusion is an UPPER BOUND. Explainer: `docs/explainers/noise-model-v2-plan.qmd`. The fitted family goes to training in `noise-v2-transfer.md` |

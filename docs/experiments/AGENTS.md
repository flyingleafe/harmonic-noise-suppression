...
| `rps-trajectory-model.md` | — (campaign code `src/experiments/rps_traj/`, drivers `scripts/rps_traj_*.py`, tables `scripts/_rps_traj_rounds_table.py`; no conf/experiment entry) | A per-rig GENERATIVE model of rotor-speed trajectories, fitted to 7 rigs (7912 s airborne) and judged against the fitted current synthesiser by a frozen five-family rule (no regression + >= 3 strict improvements). Read § "What the telemetry says" BEFORE citing a band or ACF number: DREGON `motor.measured` above 5 Hz is its own 45 Hz staircase. Sum-of-OU is excluded by the measured -23 to -61 dB/decade slope (R2, 0/7); OU + CAR(2) passes where the oscillator stays in the shoulder (R3, 1/7); R4 (Kalman likelihood) in flight. **The sampler `rps_synthesis.py` is UNTOUCHED pending review** |

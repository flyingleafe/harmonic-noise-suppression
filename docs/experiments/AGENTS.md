...
| `noise-model-v2.md` | — (studies `scripts/noise_v2_{bench_speed,corpus_survey,shaft_phase,likelihood_window,spectrogram_proxy}.py`; no conf/experiment entry) | PRE-IMPLEMENTATION phase of noise model v2 (2026-09-15, **awaiting review**, nothing submitted): the three measurement studies that set up the successor of revised-phase C1-C4. Survey adds 73 fit points over 12 rig/condition points (`noise-v2-bench-points@8f49bb0f77d3`), validated against the DREGON throttle law to 0.555 rev/s. Telemetry: the speed error is Lorentzian, not white, and leaves only a BOUNDED wobble above the label band, so C3's `D = 845 rad²/s` is LABEL error, not rotor physics. **Read § "Shaft-phase outcome" before quoting any `λ`** — `λ` is BRACKETED, not pinned (spans a factor 77 across trend-removal variants), the slope-1.5 crossing is filter-dependent, and the label-residual diffusion is an UPPER BOUND. Criteria: `T = 1.024 s` and the `ltas_abs_db` proxy at 0.70 (or 0.50) gap closure, which the current best FAILS; the composite-risk table is NOT computed. Explainer: `docs/explainers/noise-model-v2-plan.qmd` |

...
| `refined-label-training.md` | `hppnet_l2_r2refined_s0` | Is the DREGON-vs-Michael's rotor-speed gap TRAINING-LABEL noise? The quality gate's recipe with `rps_refined` as the training target and everything else — pool, validation, model, seed — held fixed. **Refuted at one seed**: DREGON cruise PIT MAE 2.07 → 2.57 while FLY124 cruise held at 0.77 → 0.78, so the cross-rig cruise gap WIDENED 1.30 → 1.79 rev/s and overall went 2.27 → 2.86. Also the `rps_key` mechanism (a `kind: frames` noise source names its label track; default byte-identical, explicit key raises instead of falling back) and the caveat that `rps_refined` is published on the 0.032 s grid while DREGON telemetry is ~929 Hz, which degrades ramp labels |

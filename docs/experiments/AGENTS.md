...
| `rig-sampler-transfer-pair.md` | `rig_{easy,hard}_scv2_unified`, `rig_easy_hppnet_l2_unified` (banks built by `scripts/_build_rig_bank.py`, sampler `src/experiments/stochastic_fit/rig_sampler.py`) | Easy and hard sim-to-real transfer (2026-09-14): training an RPS predictor on NEIGHBOURHOODS of the two rigs' cruise fits instead of the fits as points. Widening helps and the wide path cloud is not worse than the close clouds (hard 5.37, easy 5.72 best real MAE vs 9.97 point-preset, 2.99 real audio); the residual gap is all in the transitions, where both synthetic arms collapse the four rotors onto one speed. **Read § "Open decision: rebuild the banks" BEFORE launching any `rig_*` arm or rebuilding `data/rig_banks/` — two bank defects are waiting on a user decision** |
| `rps-trajectory-model.md` | — (campaign code `src/experiments/rps_traj/`, drivers `scripts/rps_traj_*.py`; no conf/experiment entry) | A per-rig GENERATIVE model of rotor-speed trajectories, fitted to 7 rigs, judged against the fitted incumbent by a frozen five-family rule. Read § "What the telemetry says" BEFORE citing a band or ACF number: DREGON `motor.measured` above 5 Hz is its own 45 Hz staircase. Sum-of-OU is excluded by the spectral slope (R2, 0/7); OU + CAR(2) reaches the shape (R3, 1/7); **best = R4**, Kalman likelihood + coloured per-rotor measurement term (1/7 PASS, michaels 5/5; dregon/pitcn/blackbird 4/5, one family each); R5: the offset must be chosen PER RIG (common vs per-rotor), unspent at the 5-round cap. Sampler `rps_synthesis.py` UNTOUCHED |
| `noise-model-v2.md` | — (studies `scripts/noise_v2_{bench_speed,corpus_survey,shaft_phase,likelihood_window,spectrogram_proxy}.py`; no conf/experiment entry) | PRE-IMPLEMENTATION of noise model v2 (2026-09-15, **awaiting review**, nothing submitted): the three studies that set up the successor of revised-phase C1-C4. Survey adds 73 fit points over 12 rig/condition points (`noise-v2-bench-points@8f49bb0f77d3`), validated against the DREGON throttle law to 0.555 rev/s. The label-free DREGON bench acoustics measure the shaft term on 12/12 recordings (`σ_ν = 1.77 rad/s`, `λ = 5.48 s⁻¹` — on `λ_ref = 6` — `D_θ = 0.599 rad²/s`), and telemetry adds only a BOUNDED wobble above the label band, so C3's `D = 845 rad²/s` is LABEL error, not rotor physics; `D_k` stays UNIDENTIFIED. **Read § "Shaft-phase outcome" before quoting any `λ`** — `λ` is BRACKETED, not pinned (spans a factor 77 across trend-removal variants), the slope-1.5 crossing is filter-dependent, and the label-residual diffusion is an UPPER BOUND. Criteria: `T = 1.024 s` and the `ltas_abs_db` proxy at 0.70 (or 0.50) gap closure, which the current best FAILS; the composite-risk table is NOT computed. Explainer: `docs/explainers/noise-model-v2-plan.qmd` |

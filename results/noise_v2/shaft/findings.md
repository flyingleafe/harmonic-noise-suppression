...
| michaels | telemetry 29 Hz | unidentified | - | - | - | - | - | - | - |
...
- **michaels** (rps, 29 Hz, 2 flights, 0 s analysed). Verdict **unidentified**: the Lorentzian fit gives `sigma_nu = - rad/s`, `lam = - 1/s` (corner - Hz, fit band 0.5-11.7 Hz), `D_theta = sigma^2/lam = - rad^2/s`, and `AIC(OU) - AIC(white) = -` (BIC -). The structure function crosses slope 1.5 at `tau = - s`, i.e. `lam = - 1/s`. The ladder step is 0.0168 rev/s, the value is held for 50.6% of samples and changes at 14.4 Hz, so `D_q = 3.21e-05 rad^2/s` = - of the measured `D_theta`; quantisation floor is below the measured diffusion.
...
| Michael's M100 | 4.21 | 1.63 | - | - | - | - | tel -x / ac -x |

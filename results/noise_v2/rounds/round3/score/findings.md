...
Top-level pass (legacy parity on both HPPNet gates): **false**. Stretch (frozen 0.70-gap DREGON target): **false**. All three frozen gates: **false**. Record `results/noise_v2/rounds/round3.json`, git `0843f3ab4b24`.
...
| michaels | equal-regime mean PIT MAE over standby/ramp/cruise (ratio of means) | 2.334258 | 3.177994 | **PASS** (margin +0.843736) | — | — (margin —) |
...
| michaels equal-regime PIT MAE | rev/s | 2.456399 | 2.334258 | -0.122140 | lower is better | **better** |
| michaels ratio vs the legacy regime mean | x | 0.8116 | 0.7712 | -0.0404 | lower is better | **better** |
| proxy ltas_abs_db, dregon cruise | dB | 3.5024 | 3.7480 | +0.2456 | lower is better | **worse** |
| proxy ltas_abs_db, michaels cruise | dB | 2.1491 | 1.8863 | -0.2628 | lower is better | **better** |
| mr_ltas, dregon cruise (report-only) | — | 2.5833 | 2.6592 | +0.0760 | lower is better | **worse** |
| mr_ltas, michaels cruise (report-only) | — | 2.2111 | 2.0460 | -0.1651 | lower is better | **better** |
| likelihood comb-band margin vs oracle | nats/s | -970.154760 | -862.834939 | +107.319821 | higher is better | **better** |
...
| HPPNet Michael's ratio | PASS | 0.771232 (2.334258 rev/s) | <= 1.05 (3.177994 rev/s) |
| proxy `ltas_abs_db` dregon_cruise | FAIL | 3.7480 dB (spread 3.2995) | <= 1.9786 dB |
| proxy `ltas_abs_db` michaels_cruise | FAIL | 1.8863 dB (spread 1.7782) | <= 1.2197 dB |
| likelihood comb (decisive) | PASS | model -372,779.3390 nats/s | oracle -371,916.5041, margin -862.8349 |
| likelihood floor | report | model -8,511.7111 nats/s | oracle -8,713.2663, margin +201.5552 |
| likelihood full | report | model -381,291.0501 nats/s | oracle -380,629.7704, margin -661.2797 |
...
|---|---:|---:|---:|---:|
| standby | 2 | 3.163707 | 0.317103 | 9.9769 |
| ramp | 1 | 3.145259 | 8.007098 | 0.3928 |
| cruise | 2 | 0.693809 | 0.755783 | 0.9180 |

Equal-regime mean 2.334258 rev/s against the frozen baseline 3.026661; ratio 0.771232 against the 1.05 bound (ratio of equally weighted per-regime MAEs (NOT a mean of ratios)).
...
| michaels_cruise | `FLY124@40.000000+8.000000` | 0.9972 | 0.0383 | 1.5145 | +0.5512 |
| michaels_cruise | `FLY124@56.000000+8.000000` | 2.7754 | 0.0803 | 2.5775 | +2.7754 |
...
michaels_cruise: mean 1.8863 dB (spread 1.7782 over 2 supports) against the closure-0.7 gate 1.2197 dB = 1.5333 - 0.7 x (1.5333 - 1.0852); margin -0.6666 dB. Secondary `mr_ltas` 2.0460 dB (reported, never decisive).
...
|---|---|---:|---:|---:|---|
| comb | 300-7900 | -372,779.3390 | -371,916.5041 | -862.8349 | True |
| floor | 30-300 | -8,511.7111 | -8,713.2663 | +201.5552 | False |
| full | 30-7900 | -381,291.0501 | -380,629.7704 | -661.2797 | True |

Frozen comb-band margin: -862.8349 nats/s (R1 freezes the comb-band model-minus-oracle margin; later rounds are held to it).
...
This section is written by `R3ScoreDregon` and is NOT produced by
`noise_v2_round_score.py --compose`; a recompose of round 3 overwrites the file
and drops it. Every cell is read verbatim out of
`results/noise_v2/rounds/round1.json` (git `066651e8eb31`),
`round2.json` (git `9579ca94592c`) and `round3.json` (git `0843f3ab4b24`) —
primary candidate per rig in each round. **No fit in any of the three rounds is
converged**, so every number below carries that label.
...
| Michael's equal-regime mean PIT MAE | rev/s | 16.799053 | 2.456399 | **2.334258** | ≤ 3.177994 |
| Michael's aggregate ratio vs legacy 3.026661 | × | 5.550358 | 0.811587 | **0.771232** | ≤ 1.05 |
| · per regime, standby (legacy 0.317103) | rev/s | 32.632045 | 3.006774 | **3.163707** | — |
| · per regime, ramp (legacy 8.007098) | rev/s | 16.955751 | 3.393256 | **3.145259** | — |
| · per regime, cruise (legacy 0.755783) | rev/s | 0.809363 | 0.969167 | **0.693809** | — |
| proxy `ltas_abs_db`, DREGON cruise | dB | 3.751843 | 3.502432 | **3.748014** | ≤ 1.978609 |
| proxy `ltas_abs_db`, Michael's cruise | dB | 1.415590 | 2.149092 | **1.886305** | ≤ 1.219668 |
...
* **Michael's** is the opposite picture: inside its parity bar since R2 and
  ...
  2.334258), with the cruise regime now BELOW its legacy baseline (ratio
  0.9180) and ramp far below it (0.3928). Standby is the one regime that got
  worse from R2 to R3 (3.006774 → 3.163707, ratio 9.9769) and it is the regime
  v2 does not fit at all — the FLY125 cruise fit drives standby on its real
  carrier.
* **The per-regime Michael's candidate is PENDING**: `R3Standby` is fitting a
  dedicated standby model (`nv2-r3-michaels-standby--6c860e`, 5 h wall) whose
  arm (`arm_michaels_v2_regimes.json`, rendered per regime via
  `--fit michaels_standby=…`) is the direct attack on that 9.9769 standby
  ratio. It is NOT in this record; when it lands, round 3 is recomposed with it
  as a second Michael's candidate and this section must be rewritten from the
  new `round3.json`.

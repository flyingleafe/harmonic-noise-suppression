...
Render seeds [2001, 2002, 2003, 2004], 8 mics, git `2537184a86bd00f9d41b940db4dffe8931c9f236`.
...
| HPPNet Michael's ratio | PASS | 0.557148 (1.686297 rev/s) | <= 1.05 (3.177994 rev/s) |
| proxy `ltas_abs_db` dregon_cruise | FAIL | nan dB (spread nan) | <= 1.9786 dB |
| proxy `ltas_abs_db` michaels_cruise | FAIL | 2.3656 dB (spread 1.0699) | <= 1.2197 dB |
| likelihood comb (decisive) | FAIL | model -349,253.2866 nats/s | oracle -371,916.5041, margin +22,663.2175 |
| likelihood floor | report | model -6,887.2568 nats/s | oracle -8,713.2663, margin +1,826.0095 |
| likelihood full | report | model -356,140.5434 nats/s | oracle -380,629.7704, margin +24,489.2270 |

## HPPNet PIT MAE per support (rev/s)

| rig | support | regime | real | candidate | seed spread |
|---|---|---|---:|---:|---:|
| michaels | `FLY124@8.000000+8.000000` | standby | 0.398474 | 0.901269 | 1.317336 |
| michaels | `FLY124@16.000000+8.000000` | standby | 0.289978 | 0.868650 | 1.627984 |
| michaels | `FLY124@27.680000+8.000000` | ramp | 3.171148 | 3.282738 | 0.715787 |
| michaels | `FLY124@40.000000+8.000000` | cruise | 0.884854 | 1.181527 | 0.110074 |
| michaels | `FLY124@56.000000+8.000000` | cruise | 0.290248 | 0.600860 | 0.124420 |
...
|---|---:|---:|---:|---:|
| standby | 2 | 0.884960 | 0.317103 | 2.7908 |
| ramp | 1 | 3.282738 | 8.007098 | 0.4100 |
| cruise | 2 | 0.891194 | 0.755783 | 1.1792 |

Equal-regime mean 1.686297 rev/s against the frozen baseline 3.026661; ratio 0.557148 against the 1.05 bound (ratio of equally weighted per-regime MAEs (NOT a mean of ratios)).

## Proxy `ltas_abs_db` per support (dB, mic 0, absolute level)

| group | support | ltas_abs_db | seed spread | mr_ltas | level offset |
|---|---|---:|---:|---:|---:|
| michaels_cruise | `FLY124@40.000000+8.000000` | 1.8307 | 1.0358 | 2.2440 | +0.9548 |
| michaels_cruise | `FLY124@56.000000+8.000000` | 2.9005 | 1.5845 | 3.1410 | +2.9005 |
...
michaels_cruise: mean 2.3656 dB (spread 1.0699 over 2 supports) against the closure-0.7 gate 1.2197 dB = 1.5333 - 0.7 x (1.5333 - 1.0852); margin -1.1459 dB. Secondary `mr_ltas` 2.6925 dB (reported, never decisive).
...
|---|---|---:|---:|---:|---|
| comb | 300-7900 | -349,253.2866 | -371,916.5041 | +22,663.2175 | False |
| floor | 30-300 | -6,887.2568 | -8,713.2663 | +1,826.0095 | False |
| full | 30-7900 | -356,140.5434 | -380,629.7704 | +24,489.2270 | False |

Frozen comb-band margin: +22,663.2175 nats/s (R1 freezes the comb-band model-minus-oracle margin; later rounds are held to it).

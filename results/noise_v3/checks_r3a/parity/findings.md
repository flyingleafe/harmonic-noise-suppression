...
Render seeds [2001, 2002, 2003, 2004], 8 mics, git `1e45a206154c18ea0f48a94af7e24c2df19892a6`.
...
| HPPNet Michael's ratio | PASS | 0.553128 (1.674130 rev/s) | <= 1.05 (3.177994 rev/s) |
| proxy `ltas_abs_db` dregon_cruise | FAIL | nan dB (spread nan) | <= 1.9786 dB |
| proxy `ltas_abs_db` michaels_cruise | FAIL | 2.3701 dB (spread 0.2209) | <= 1.2197 dB |
| likelihood comb (decisive) | FAIL | model -333,321.5198 nats/s | oracle -371,916.5041, margin +38,594.9843 |
| likelihood floor | report | model -7,023.9041 nats/s | oracle -8,713.2663, margin +1,689.3622 |
| likelihood full | report | model -340,345.4238 nats/s | oracle -380,629.7704, margin +40,284.3466 |

## HPPNet PIT MAE per support (rev/s)

| rig | support | regime | real | candidate | seed spread |
|---|---|---|---:|---:|---:|
| michaels | `FLY124@8.000000+8.000000` | standby | 0.398474 | 1.029292 | 1.491687 |
| michaels | `FLY124@16.000000+8.000000` | standby | 0.289978 | 0.759244 | 0.813173 |
| michaels | `FLY124@27.680000+8.000000` | ramp | 3.171148 | 3.306106 | 0.241050 |
| michaels | `FLY124@40.000000+8.000000` | cruise | 0.884854 | 1.108183 | 0.091465 |
| michaels | `FLY124@56.000000+8.000000` | cruise | 0.290248 | 0.535849 | 0.083980 |
...
|---|---:|---:|---:|---:|
| standby | 2 | 0.894268 | 0.317103 | 2.8201 |
| ramp | 1 | 3.306106 | 8.007098 | 0.4129 |
| cruise | 2 | 0.822016 | 0.755783 | 1.0876 |

Equal-regime mean 1.674130 rev/s against the frozen baseline 3.026661; ratio 0.553128 against the 1.05 bound (ratio of equally weighted per-regime MAEs (NOT a mean of ratios)).

## Proxy `ltas_abs_db` per support (dB, mic 0, absolute level)

| group | support | ltas_abs_db | seed spread | mr_ltas | level offset |
|---|---|---:|---:|---:|---:|
| michaels_cruise | `FLY124@40.000000+8.000000` | 2.4805 | 0.7131 | 2.6838 | -0.0490 |
| michaels_cruise | `FLY124@56.000000+8.000000` | 2.2596 | 0.8609 | 2.7504 | +1.9019 |
...
michaels_cruise: mean 2.3701 dB (spread 0.2209 over 2 supports) against the closure-0.7 gate 1.2197 dB = 1.5333 - 0.7 x (1.5333 - 1.0852); margin -1.1504 dB. Secondary `mr_ltas` 2.7171 dB (reported, never decisive).
...
|---|---|---:|---:|---:|---|
| comb | 300-7900 | -333,321.5198 | -371,916.5041 | +38,594.9843 | False |
| floor | 30-300 | -7,023.9041 | -8,713.2663 | +1,689.3622 | False |
| full | 30-7900 | -340,345.4238 | -380,629.7704 | +40,284.3466 | False |

Frozen comb-band margin: +38,594.9843 nats/s (R1 freezes the comb-band model-minus-oracle margin; later rounds are held to it).

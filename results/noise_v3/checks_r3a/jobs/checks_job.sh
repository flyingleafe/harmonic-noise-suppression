...
O=results/noise_v3/checks_r3a; L=$O/logs; mkdir -p "$L" "$O/parity" "$O/proxy_seeds"
...
echo "V3CHK job=$JOB tag=r3a fits=results/noise_v3/fits_r3a out=$O sha=$(git rev-parse --short=12 HEAD 2>/dev/null) host=$(hostname) nproc=$(nproc)"
...
chk() { $PY scripts/noise_v3_checks.py "$1" --fits results/noise_v3/fits_r3a --out $O --keys "$2"; }
...
( step tonality $PY scripts/noise_v2_tonality_audit.py --out $O/tonality --fit dregon=results/noise_v3/fits_r3a/dregon_room2_floor__flight_v3.json --fit michaels=results/noise_v3/fits_r3a/michaels_fly125_cruise__flight_v3.json --fit michaels_standby=results/noise_v3/fits_r3a/michaels_fly125_standby__flight_v3.json ) & P="$P $!"
( step parity_dregon $PY scripts/noise_v2_round_score.py --round 6 --fits results/noise_v2/rounds/round3/fits --rigs dregon --fit dregon=results/noise_v3/fits_r3a/dregon_room2_floor__flight_v3.json --candidate dregon_v3 --arm-out $O/parity/arm_dregon_v3.json ) & P="$P $!"
( step parity_michaels $PY scripts/noise_v2_round_score.py --round 6 --fits results/noise_v2/rounds/round3/fits --rigs michaels --fit michaels=results/noise_v3/fits_r3a/michaels_fly125_cruise__flight_v3.json --fit michaels_standby=results/noise_v3/fits_r3a/michaels_fly125_standby__flight_v3.json --candidate michaels_v3 --arm-out $O/parity/arm_michaels_v3.json ) & P="$P $!"
( for n in 0 1 2 3; do step proxy_dregon_s$n $PY scripts/noise_v2_round_score.py --round 6 --fits results/noise_v2/rounds/round3/fits --rigs dregon --no-probe --fit dregon=results/noise_v3/fits_r3a/restarts/dregon_room2_floor__flight_v3__s$n.json --candidate dregon_r3a_s$n --arm-out $O/proxy_seeds/arm_dregon_r3a_s$n.json; done ) & P="$P $!"
( for n in 0 1 2 3; do step proxy_michaels_s$n $PY scripts/noise_v2_round_score.py --round 6 --fits results/noise_v2/rounds/round3/fits --rigs michaels --no-probe --fit michaels=results/noise_v3/fits_r3a/restarts/michaels_fly125_cruise__flight_v3__s$n.json --fit michaels_standby=results/noise_v3/fits_r3a/restarts/michaels_fly125_standby__flight_v3__s$n.json --candidate michaels_r3a_s$n --arm-out $O/proxy_seeds/arm_michaels_r3a_s$n.json; done ) & P="$P $!"
wait $P

...
F=results/noise_v3/diag/rig/B; C=results/noise_v2/rounds/round1/supports; mkdir -p "$F" "$C"
exec > >(tee -a "$F/dregon_room2_floor_job.log") 2>&1
T0=$(date +%s)
...
DEST="s3://omnirun-artifacts/$JOB/outputs/results/noise_v3/diag/rig/B"
...
S=/tmp/supports_dregon_room2_floor; mkdir -p "$S"
$PY scripts/noise_v2_supports.py build --set dregon-floor --out "$S" > "$F/dregon_room2_floor_build.log" 2>&1; EB=$?
tail -2 "$F/dregon_room2_floor_build.log"
for f in "$S"/*.npz; do b=$(basename "$f"); [ -f "$C/$b" ] || { cp "$f" "$C/.tmp.$$.$b" && mv -f "$C/.tmp.$$.$b" "$C/$b"; }; done
echo "V3DIAG build exit=$EB wall_s=$(( $(date +%s) - T0 ))"
$PY -u scripts/noise_v3_diag_rig.py --fit results/noise_v3/fits_r2/dregon_room2_floor__flight_v3.json --set dregon-floor --wind \
  --out "$F" --steps fit,zeromean,notch --rig-wall-s 540 2>&1 | grep -v Warn; EF=${PIPESTATUS[0]}

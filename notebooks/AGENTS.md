...
| `noise_lab.ipynb` | **Every generation of the rotor-noise model, on ONE trajectory.** Merges the deleted `stochastic_noise_lab` and `noise_v2_sampler`: pick a trajectory once (`NL.trajectory` — the fitted model of any `rps-traj-fits` rig or the `posterior` hyperprior, a real recording's telemetry, or the `rps_synthesis` scaffold: OU, intermittent, whole flight), then render **every** generation on it with one seed and one level rule — `LegacyRandom` (a fresh `stochastic_rotor_noise` draw), `LegacyFit` (a stage-2 **anchor** fit on the physical scale — `rig_sampler.FIT_PATHS` via `load_anchor`, built as a bank entry at zero perturbation with the donor policy's per-clip dynamics (`rig_sampler.entry_params`, `dynamics="donor"`; `"none"` keeps the drift-free export) — the measured rig each legacy bank was drawn around, i.e. the 9.97 point-preset end of the 5.37/5.72 bank arms; `legacy_fit_names()`), `LegacyBank` (the exact `data/rig_banks/rig_{easy,hard}_n2048.json` entries the `rig_easy`/`rig_hard` arms trained on), `V2Fit` (the R5 uncalibrated DREGON fit, the R3 Michael's standby+cruise pair) and `V2Bank` (`noise_v2_{easy,hard}_n2048.json` = `dload:noise-v2-banks`) — so the generations compare by ear (`players`), by spectrogram (`show`) and by number (`line_stats`: the R4 order prominence over the local floor). `NL.tune(source, traj)` is the **old lab's slider panel**, kept: amplitude means, wander dB / wander time, harmonic coherence, floor-colour wander, recording floor, line mode, clip length, new-params / regenerate — over the *shared* trajectory and level rule (a bank entry is read-only, a v2 payload exposes `comb_offset_db` alone, because both are fitted). `model_vs_realised` reads either generation's model against its clip. Rendering only: no forward model is ever evaluated on a fit pool. Logic lives in `noise_lab.py`. |

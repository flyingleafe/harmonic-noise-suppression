...
| `noise_v2_sampler.ipynb` | **Sample rendered clips from the two winning noise-model-v2 fits.** Pick a rig (`dregon` — the R5 **uncalibrated** single-regime fit, `render.render_noise`; `michaels` — the R3 standby+cruise pair, `render.render_noise_regimes` with the 45–65 rev/s smoothstep), sample a rotor-speed trajectory from the **fitted trajectory model of the same rig** (`data_processing.trajectory_model`, the `rps-traj-fits` bundle, `measurement_noise=False`) — airborne-only or a whole `wrap_airborne` flight, or from the rig **hyperprior** (`traj_rig="posterior"`, a fresh drone per draw) — render it, look at it, hear it, and read the fit's expected periodogram against the realised one. Rendering only: no forward model is ever evaluated on a fit pool. Logic lives in `noise_v2_sampler.py`. |

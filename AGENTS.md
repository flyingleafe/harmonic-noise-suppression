...
4. **Worktrees.** Implementation work happens in `.worktrees/<name>` on a
   ...
   and opens a shell there (`-n` only prints the path; re-run it to relink new
   `results/` dirs). The `.venv` is shared — never `uv sync` in a worktree.
   Start the agent **with cwd inside the worktree**; never edit `.worktrees/*`
   paths from the main checkout — the worktree's own `src/` must be first on

...
| `validation.py` | Full-panel RPS validation: one concatenated finite loader, GPU-vectorized MAE-optimal PIT, zero-copy nested index views, domain aggregates, log-scale median progress, any-subset LR/stopping state. The prediction is read through a task-dispatched `RPSReadout` (`rps_readout_for`): `rps_prediction` takes `rps_pred` as is, `salience_rps` calls the model's on-device `decode_logits` (the deployed decoder), so both task families share the panel, the controller and the subset-best checkpoints. |

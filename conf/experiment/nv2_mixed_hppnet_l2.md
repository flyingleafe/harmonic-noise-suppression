---
experiment: nv2_mixed_hppnet_l2
training_config: conf/experiment/nv2_mixed_hppnet_l2.yaml
...
`decode_logits`) on the noise-model-v2 **MIXED** stream — the mixed arm of the
...
**5.37**; and the same-stream arm of the other trunk (`nv2_mixed_scv2`), which
...
`data/rig_banks/noise_v2_hard_n2048.json` (rebuilt in-job with `python
scripts/noise_v2_build_bank.py --preset hard`). Structure is
...
checkpoint — `python scripts/_regime_decomp.py --exp nv2_mixed_hppnet_l2 --ckpt
...
```bash
scripts/noise_v2_submit_arms.sh nv2_mixed_hppnet_l2
```

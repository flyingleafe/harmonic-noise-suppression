---
experiment: nv2_easy_ft_scv2
training_config: conf/experiment/nv2_easy_ft_scv2.yaml
batch: docs/experiments/noise-v2-transfer.md
---

## Motivation

Stage 2 of the easy curriculum for the direct regressor (`real_r1_scv2`
architecture, `hb_scv2_mag_nogate` head): `nv2_easy_scv2` (trained on the
noise-model-v2 EASY bank, synthetic only) fine-tuned on REAL audio.
...
Paired with `nv2_hard_ft_scv2`: the two differ only in which synthetic
...
Warm start: `checkpoint: best:real_overall@nv2_easy_scv2`, which
`training.loop` resolves to
`r2://<bucket>/<prefix>/nv2_easy_scv2/checkpoints/best_real_overall.ckpt` and
...
**Ordering constraint:** stage 1 (`nv2_easy_scv2`) must have finished and
...
`conf/experiment/nv2_easy_scv2.md`; this arm itself trains on real audio and
builds no bank.
...
checkpoint — `python scripts/_regime_decomp.py --exp nv2_easy_ft_scv2 --ckpt
...
```bash
scripts/noise_v2_submit_arms.sh nv2_easy_ft_scv2
```
